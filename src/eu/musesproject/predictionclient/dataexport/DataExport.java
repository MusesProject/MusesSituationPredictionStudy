package eu.musesproject.predictionclient.dataexport;
...
import eu.musesproject.predictionclient.preferences.LabelingStatePreference;
import eu.musesproject.predictionclient.preferences.defaultpreferences.IsLabelingActivatedPreference;
import eu.musesproject.predictionclient.session.SessionDataController;
...
public class DataExport {
	...
	public void exportData() {
		...
		if (IsLabelingActivatedPreference.getInstance(mContext).get()) {
	...
	private class ExportTask extends AsyncTask<Void, Void, Void> {
		...
		private boolean createFileFromDb() {
			...
			File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
					.getPath() + File.separatorChar + mSubDir);
			...
			try {
				...
				if (cursor.moveToFirst()) {
					do {
						csvFile.append(cursor.getInt(cursor.getColumnIndex(DBManager.SESSION_ID_USERSELECTION_LABELING))
								+ ";");
						csvFile.append(cursor.getString(cursor.getColumnIndex(DBManager.VALUE_USERSELECTION_LABELING))
								+ ";");
						csvFile.append(cursor.getInt(cursor.getColumnIndex(DBManager.ID_CONTEXTEVENT_LABELING)) + ";");
						csvFile.append(cursor.getString(cursor.getColumnIndex(DBManager.TYPE_CONTEXTEVENT_LABELING))
								+ ";");

						csvFile.append(cursor.getLong(cursor.getColumnIndex(DBManager.TIMESTAMP_CONTEXTEVENT_LABELING))
								+ ";");
						csvFile.append(cursor.getString(cursor.getColumnIndex(DBManager.KEY_PROPERTY_LABELING)) + ";");
						csvFile.append(cursor.getString(cursor.getColumnIndex(DBManager.VALUE_PROPERTY_LABELING)) + ";");
						csvFile.append("\n");
		...
		protected void onCancelled() {
			Toast.makeText(mContext, "File could not be exported", Toast.LENGTH_LONG).show();
			super.onCancelled();
		}
		...
		protected void onPostExecute(Void result) {
			...
			Toast.makeText(mContext, "File with labeled data was exported to the downloads folder of your smartphone",
